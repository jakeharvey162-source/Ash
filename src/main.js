...
let heroVisualCleanup=null,pairing=null,pairingTimer=null,syntheticWaveRaf=0,activeAudio=null,activeAudioUrl="",activeAudioCleanup=null,voiceQueue=[],voiceQueueRunning=false,voiceGeneration=0,typeGeneration=0,premiumVoiceUnavailable=false;
...
async function createComputerControlJob(){
  const prompt=document.querySelector("#computerGoal")?.value.trim();
  const target=document.querySelector("#computerDevice")?.value||"";
  if(!prompt)return toast("Describe what you want Ash to do on the computer.");
  if(!target)return toast("Choose an online desktop first.");
  const device=devices.find(d=>d.id===target);
  if(!device?.capabilities?.computer_control)return toast("Computer control is not enabled on that desktop. Turn on ASH_COMPUTER_CONTROL=1 locally first.");
  await supa("/rest/v1/jarvis_remote_jobs",{
    method:"POST",
    headers:{Prefer:"return=representation"},
    body:JSON.stringify({
      ...
      mode:"high",
      payload:{prompt,computer_control:true,source:"user",executor:"desktop"},
      status:"queued",
      requires_confirmation:true
    })
  });
  await loadOps(true);view="activity";render();
  toast("Computer task queued. Review it in Activity, then approve it to run.");
}
...
function bind(){
  ...
  document.querySelector("#createComputerControl")?.addEventListener("click",createComputerControlJob);
